...
if (isset($_POST['validate'])) {
	...
	if (!empty($_POST['nom']) && !empty($_POST['email']) && !empty($_POST['contact']) && !empty($_POST['password'])) {
		...
		$checkIfUserAlreadyExists = $BDD->prepare("SELECT nom_client FROM clients WHERE nom_client = ?");
		$checkIfUserAlreadyExists->execute(array($nom_client));

		if ($checkIfUserAlreadyExists->rowCount() == 0) {

			// Insérer l'utilisateur dans la base de données
			$insertUserOnWebsite = $BDD->prepare(
				"INSERT INTO clients(nom_client, email_client, contact_client, motdepasse_client) VALUES(?, ?, ?, ?)"
			);
			$insertUserOnWebsite->execute(array($nom_client, $email_client, $contact_client, $motdepasse_client));

			// Récupérer les informations de l'utilisateur
			$getInfoUser = $BDD->prepare("SELECT id, nom_client, email_client, contact_client FROM clients WHERE nom_client = ? AND email_client = ? AND contact_client = ?");
			$getInfoUser->execute(array($nom_client, $email_client, $contact_client));
